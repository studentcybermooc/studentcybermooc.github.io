...
- Why using a JWT instead of Session ?
    ...
    - I'll also release a version of this app without JWT.
...
### 1.4 - JWT verification decorator
...
The route `/api/v1/need_login` is accessible only if `login_required` succeeds, so if the user provieds a valid token.
...
If you're stuck or don't understand something, feel free to drop [me an email / dm on twitter](/authors/gmolveau/) / a comment below. You can also take a look at `flask_learning/flask_cybermooc/version_5` to see the reference code. And use `reset.sh` to launch it.
